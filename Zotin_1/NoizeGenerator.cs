...
namespace Zotin_1
{
    class NoizeGenerator
    {
        ...
        public Bitmap applyNoise(Bitmap originalImage, string mode, string colorMode, int noizeLevel,  double k, int additionalK = 0, int secondAdditionalK = 0, int colorChannel = 0)
        {
        ...
        public Bitmap applyAdditiveNoize(string colorMode, Bitmap originalImage, int noizeLevel, int min, int max, int colorChannel = 0)
        {
            Bitmap result = new Bitmap(originalImage);
            int noisepixel = (int)(originalImage.Width * originalImage.Height * noizeLevel / 100);
            int x, y, randomValue;
